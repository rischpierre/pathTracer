...
struct Face {
  explicit Face(Eigen::Vector3f v0 = Eigen::Vector3f(),
                ...
                Eigen::Vector3f n2 = Eigen::Vector3f(), int id = 0,
                uint meshId = 0, uint shaderId = 0)
      : v0(std::move(v0)), v1(std::move(v1)), v2(std::move(v2)),
        nf(std::move(nf)), n0(std::move(n0)), n1(std::move(n1)),
        n2(std::move(n2)), id(id), meshId(meshId), shaderId(shaderId) {}
  Face(const Face &f) {
    v0 = f.v0;
    v1 = f.v1;
    v2 = f.v2;
...
struct BBox {
  ...
  bool isFaceInside(const Face &face) const {
    ...
    if (v0.x() >= min.x() && v0.x() <= max.x() && v0.y() >= min.y() &&
        v0.y() <= max.y() && v0.z() >= min.z() && v0.z() <= max.z()) {
      return true;
    }
    if (v1.x() >= min.x() && v1.x() <= max.x() && v1.y() >= min.y() &&
        v1.y() <= max.y() && v1.z() >= min.z() && v1.z() <= max.z()) {
      return true;
    }

    if (v2.x() >= min.x() && v2.x() <= max.x() && v2.y() >= min.y() &&
        v2.y() <= max.y() && v2.z() >= min.z() && v2.z() <= max.z()) {
      return true;
    }
    return false;
  }
...
struct Mesh {
  Mesh(std::vector<Face> &faces, const std::string &name, const BBox &bbox,
       uint id)
...
public:
  ...
  void parsePrimsByType(pxr::UsdPrim &prim, const pxr::UsdStage &stage,
