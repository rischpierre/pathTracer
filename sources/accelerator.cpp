...
void Accelerator::print(const BVHNode &node, int depth) {
  std::cout << std::string(depth, '.') << "node" << node.id
            << " fCount: " << node.facesID.size();
...
void Accelerator::exportBBoxesToUsd(const std::string &filePath) const {
  ...
  for (const auto &node : allNodes) {
    ...
    auto mesh = pxr::UsdGeomMesh::Define(
        stage, pxr::SdfPath("/bbox" + std::to_string(node.id)));
...
void Accelerator::getIntersectedNodesRecursive(
    const BVHNode &node, const Ray &ray, std::vector<BVHNode> *nodes) const {
