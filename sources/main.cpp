...
Ray createCameraRay(const Camera &cam, int x, int y) {
  ...
  float sampleX =
      (cam.hAperture / (float)RESOLUTION_W) * (float)x - cam.hAperture / 2;
  // todo why - for the y sample ?
  float sampleY =
      -((cam.vAperture / (float)RESOLUTION_H) * (float)y - cam.vAperture / 2);
  float sampleZ = -cam.focalLength; // camera is pointing towards - z

  pxr::GfVec3f rayOrig = cam.toWorld.Transform(pxr::GfVec3f(0));
  pxr::GfVec3f rayDir =
      cam.toWorld.TransformDir(pxr::GfVec3f(sampleX, sampleY, sampleZ))
          .GetNormalized();
...
int main(int argc, char *argv[]) {
  ...
  scene.print();
  clock_t t2 = clock();
  std::cout << "Parsed scene in " << (float)(t2 - t1) / CLOCKS_PER_SEC
            << " seconds" << std::endl;
  ...
  t2 = clock();
  std::cout << "Build acceleration structure in "
  ...
  std::cout << "Multi threaded" << std::endl;
    ...
    for (int x = 0; x < RESOLUTION_W; x++) {
      ...
      if (progress % ((RESOLUTION_H * RESOLUTION_W) / 10) == 0) { // every 10%
        int percentage =
            int((float)progress * 100 / (RESOLUTION_H * RESOLUTION_W));
    ...
    std::cout << "Rendered in " << (t4 - t3).seconds() << " seconds"
              << std::endl;
