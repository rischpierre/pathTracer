...
void Scene::parseLights(const std::vector<pxr::UsdPrim> &usdLights) {
  ...
  for (const pxr::UsdPrim &light : usdLights) {
    ...
    pxr::UsdAttribute colorAttr =
        light.GetAttribute(pxr::TfToken("inputs:color"));
    pxr::UsdAttribute intensityAttr =
        light.GetAttribute(pxr::TfToken("inputs:intensity"));
    pxr::UsdAttribute widthAttr =
        light.GetAttribute(pxr::TfToken("inputs:width"));
    pxr::UsdAttribute heightAttr =
        light.GetAttribute(pxr::TfToken("inputs:height"));
    ...
    pxr::GfVec3f normal(0, 0,
                        1); // lights are pointing down the Z axis by default
...
Shader Scene::createShader(const pxr::UsdGeomMesh &mesh) {
  ...
  pxr::UsdPrim materialPrim =
      bindingApi.GetPrim().GetStage()->GetPrimAtPath(material);
  ...
  for (auto descendant : descendants) {
    ...
    pxr::UsdAttribute diffuseAttr =
        descendant.GetAttribute(pxr::TfToken("inputs:diffuseColor"));
...
void Scene::convertUSDMeshes(const std::vector<pxr::UsdPrim> &usdMeshes) {
  ...
  for (auto &primMesh : usdMeshes) {
    ...
    pxr::GfBBox3d bbox =
        usdMesh.ComputeWorldBound(STATIC_FRAME, pxr::TfToken("default"));
    ...
    Eigen::Vector3f bboxMinE = Eigen::Vector3f(
        (float)bboxMin[0], (float)bboxMin[1], (float)bboxMin[2]);
    Eigen::Vector3f bboxMaxE = Eigen::Vector3f(
        (float)bboxMax[0], (float)bboxMax[1], (float)bboxMax[2]);
    ...
    for (auto &fCount : fVertexCounts) {
      if (fCount > 3) {
        std::cout
            << "Failure: face with more than 3 vertices (not implemented yet)"
            << std::endl;
        exit(1);
      }
    }
    ...
    for (int vId = 0; vId < fVertexIds.size(); vId = vId + 3) {
      ...
      auto n1 = pxr::GfVec3f(normals[vId + 1][0], normals[vId + 1][1],
                             normals[vId + 1][2]);
      auto n2 = pxr::GfVec3f(normals[vId + 2][0], normals[vId + 2][1],
                             normals[vId + 2][2]);
...
void Scene::parsePrimsByType(pxr::UsdPrim &prim, const pxr::UsdStage &stage,
                             std::vector<pxr::UsdPrim> &rPrims,
...
void RectLight::computeSamples() {
  ...
  int i = 0;
  for (float x = -this->width / 2 + stepSizeWidth / 2;
       x < this->width / 2 - stepSizeWidth / 2; x += stepSizeWidth) {
    for (float y = -this->height / 2 + stepSizeHeight / 2;
         y < this->height / 2 - stepSizeHeight / 2; y += stepSizeHeight) {

      auto sample = pxr::GfVec3f(x, y, 0);
      float jitter =
          (((float)rand() / (float)RAND_MAX) * 2) - 1; // range: [-1, 1]

      sample = pxr::GfVec3f(sample[0] + stepSizeWidth * jitter,
                            sample[1] + stepSizeHeight * jitter, 0);
