...
ShadingPoint Integrator::computeShadingPoint(float u, float v, const Face &face,
                                             const Eigen::Vector3f &hitPoint,
                                             const Scene &scene) {

  // Gouraud's smooth shading technique
  Eigen::Vector3f smoothNormal =
      ((1 - u - v) * face.n0 + u * face.n1 + v * face.n2).normalized();

  return ShadingPoint{
      hitPoint, smoothNormal, face, u, v, scene.getShaderFromFace(face)};
}

Eigen::Vector3f
Integrator::getDirectContribution(const Ray &ray, const Scene &scene,
  ...
  for (const RectLight &light : scene.rectLights) {
    ...
    for (const Eigen::Vector3f &lightSample : light.samples) {
      ...
      Eigen::Vector3f lightDirSample =
          (lightSample - shadingPoint.hitPoint).normalized();

      if (lightDirSample.dot(light.normal) <
          0) // skip if samples is not visible to the shading point
        continue;

      // to avoid shadow acne
      Eigen::Vector3f shadowRayOrigin =
          shadingPoint.hitPoint + shadingPoint.n * SHADOW_BIAS;
      ...
      for (const Face &face : accelerator.getIntersectedFaces(shadowRay)) {
        ...
        float u, v, distance;
        intersected =
            isRayIntersectsTriangle(&shadowRay, &face, &distance, u, v);
        if (intersected &&
            distance < (lightSample - shadingPoint.hitPoint).norm()) {
          break;
        }
      }
...
Eigen::Vector3f Integrator::createHemisphereSample(const float &r1,
                                                   const float &r2) {
...
void createCoordinateSystemFromNormal(const Eigen::Vector3f &N,
                                      Eigen::Vector3f &r_Nt,
                                      Eigen::Vector3f &r_Nb) {
  ...
  if (fabs(N.x()) > fabs(N.y()))
    r_Nt = Eigen::Vector3f(N.z(), 0, -N.x()) /
           sqrtf(N.x() * N.x() + N.z() * N.z());
  else
    r_Nt = Eigen::Vector3f(0, -N.z(), N.y()) /
           sqrtf(N.y() * N.y() + N.z() * N.z());

  r_Nb = N.cross(r_Nt);
}

Eigen::Vector3f Integrator::castRay(const Ray &ray, const Scene &scene,
                                    uint depth) {
  ...
  ShadingPoint shdPoint = computeShadingPoint(nearestFaceU, nearestFaceV,
                                              *nearestFace, hitPoint, scene);

  Eigen::Vector3f directContribution =
      getDirectContribution(ray, scene, shdPoint);

#if DEBUG_PIXEL == true
  std::string depthTab(depth * 2, ' ');
  std::cout << depthTab << "DC: " << directContribution[0] << " ";
  std::cout << directContribution[1] << " " << directContribution[2]
            << " mesh: ";
  ...
  for (uint i = 0; i < INDIRECT_SAMPLES; i++) {
    ...
    Eigen::Vector3f sampleWorld = {
        ...
        sample.x() * Nb.z() + sample.y() * shdPoint.n.z() +
            sample.z() * Nt.z()};
  ...
  hitColor = (directContribution / M_PI + 2 * indirectContribution)
                 .cwiseProduct(shdPoint.shader.diffuse);

#if DEBUG_PIXEL == true
  std::cout << depthTab << "IC: " << indirectContribution[0] << " ";
  std::cout << indirectContribution[1] << " " << indirectContribution[2]
            << std::endl;
#endif
